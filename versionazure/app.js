...
if (data.results[0].isFinal) {
   ...
   let consoletext = (data.results[0] && data.results[0].alternatives[0]) ?
            `Transcription: ${data.results[0].alternatives[0].transcript}\n` :
            `\n\nReached transcription time limit, press Ctrl+C\n`
    client.emit('consoletext', consoletext);
...
server.listen(port, "10.0.0.4", function () { //http listen, to make socket work
    // app.address = "149.248.61.13";
    console.log('Server started on port:' + port)
    console.log('Server web address:' + 'http://40.85.252.154:' +port)
});